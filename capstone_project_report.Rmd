...
ggsave('calories_vs_activity.png')
```
...
ggsave("distribute_calories_per_step.png")
```
...
ggsave("calories_vs_steps_segments.png")
```
...
ggsave("bodywise_calories_vs_steps.png")
```
...
ggsave("distribute_avg_daily_activity.png")
```
**Findings:**
...
ggsave("less_cal_individual_trends.png")
```
...
ggsave("zero_steps_distribution.png")
```
...
ggsave("device_time_distribution.png")
```
...
ggsave("activity_time_distribution.png")
```
...
ggsave("daywise_light_activity.png")
```
...
ggsave("daywise_moderate_activity.png")
```
...
ggsave("daywise_intense_activity.png")
```
...
ggsave("sleep_time_distribution.png")
```
...
ggsave("less_sleepy_consumers.png")
```
...
ggsave("wake_time_distribution.png")
```
...
ggsave("hr_comprehensiveness.png")
```
...
ggsave("body_data_availability.png")
```
...
## Product Improvement  
  ...
  1. Adding feature of periodic reminder by the tracker to take break in case of sensing extended 
  sitting may help improve metabolic health of consumer by cutting down prolonged sedentary time.
  2. Adding feature of sending alerts to user's phone in the event of sensing no-use, in order to      remind her to wear it. This may attract forgetful consumers to buy.


# Limitations
The insights were drawn using usage-data of only 41 individuals. Hence, the conclusions drawn would lack statistical significance. 
...
```{r, message = FALSE, warning = FALSE}
num_of_consumers <- c(41,28,17,17)
 data_frame <- c("activity", "sleep", "heartrate", "body")
df <- data.frame(num_of_consumers,data_frame)
ggplot(df, aes(x = data_frame, y = num_of_consumers, fill = data_frame)) + geom_col() + 
labs(x = 'Data', y = 'Number of participants', title = 'Number of consumers under analysis')+
  geom_text(aes(label=num_of_consumers, vjust = -.25), size = 3)+
theme(legend.position = "none")

ggsave("num_of_consumers.png")
```
